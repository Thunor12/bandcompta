use bandcompta_shared::{ApiClient, DateFilter, Transaction, TreasurySummary};
use wasm_bindgen_futures::spawn_local;
use web_sys::wasm_bindgen::JsCast;
use web_sys::HtmlInputElement;
use yew::prelude::*;

use crate::ui::{format_euro, LoadState, TransactionTypeUi};
                        ...
                        <tr class={row_class} onclick={on_select}>
                            ...
                            <td>{
                                if transaction.invoice_path.is_empty() {
                                    html! { <span class="muted">{ "Manquant" }</span> }
                                } else {
    ...
    html! {
        <div class="details-panel">
            ...
            <dl>
                ...
                <dt>{ transaction.transaction_type.justification_label() }</dt>
                <dd>{
                    if transaction.invoice_path.is_empty() {
...
pub fn explorer() -> Html {
    ...
    let on_refresh = {
        ...
        Callback::from(move |_| {
            load_data(date_filter.clone(), transactions.clone(), summary.clone());
        })
    };
    ...
    html! {
        <>
            <header class="page-header">
                <div>
                    <h1>{ "Explorateur" }</h1>
                    <p class="subtitle">{ "Transactions et trésorerie" }</p>
                </div>
